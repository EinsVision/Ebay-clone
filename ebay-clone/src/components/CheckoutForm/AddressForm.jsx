...
function AddressForm({checkoutToken}) {
  ...
  const fetchSubdiisions = async (countryCode) => {
    const { subdivisions } = await commerce.services.localeListSubdivisions(countryCode);
    setShippingSubdivisions(subdivisions);
    setShippingSubdivision(Object.keys(subdivisions)[0]);
  }

  const fetchShippingOptions = async (checkTokenId, country, region = null) => {
  ...
  useEffect( () => {
    if(shippingCountry) {
      fetchSubdiisions(shippingCountry);
    }
  }, [shippingCountry]);
      ...
      <FormProvider {...methods}>
        <form onSubmit=''>
          <Grid container spacing={3}>
            <CustomTextField name='firstName' label='Fist name' />
